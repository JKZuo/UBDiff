...
# Construction of the user-specific POI transition graph G_u
def getSeqGraph(seq, time_list):
    i, x_and_t, senders, nodes = 0, [], [], {}
    for node, time in zip(seq, time_list):
        if node not in nodes:
            nodes[node] = i
            x_and_t.append([node, time])
            i += 1
        senders.append(nodes[node])
    x_and_t = torch.LongTensor(x_and_t)
    ...
    return Data(x=x_and_t, edge_index=edge_index, num_nodes=len(nodes), mean_interv=mean_interv, edge_time=time_interv, edge_dist=dist_interv)
...
def getDatasets(path='../data/processed', dataset='IST'):
    ...
    return n_user, n_poi, (trn_ds, val_ds, tst_ds)